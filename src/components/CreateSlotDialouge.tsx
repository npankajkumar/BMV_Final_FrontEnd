...
const HCard = () => {
  ...
  const handleFromTimeSelection = (timeString: string) => {
    setFromDate((curr) => {
      const splitedArr = timeString.split(":");
      return set(curr, {
        hours: parseInt(splitedArr[0]),
        minutes: parseInt(splitedArr[1].substring(0, 2)),
      });
    });
    ...
    <Card>
      ...
      <CardContent className="space-y-2">
        <div className="space-y-1 flex gap-4">
          ...
          />
          <TimeSelection
            from={add(date, { minutes: 1 })}
            handleTimeSelection={handleFromTimeSelection}
          />
          <TimeSelection
            from={add(date, { minutes: 15 })}
            handleTimeSelection={handleToTimeSelection}
          />
        </div>
...
const TimeSelection = ({
  ...
  from: Date;
  handleTimeSelection: (timeString: string) => void;
}) => {
