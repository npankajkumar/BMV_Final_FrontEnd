...
import { BarChartComponent, EarningsChart } from '@/components/line-chart/LineChart'
...
const Home = () => {
  ...
  return (
    <div className='w-vw p-4'>
        <div className='flex items-center justify-between gap-4'>
        <CardTitle>Dashboard</CardTitle>
        <Button variant={"outline"}><Plus/>Add Venue</Button>
        </div>
        <div className='grid grid-cols-1 lg:grid-cols-4 gap-4 my-5'>
          ...
          <Card className="h-full p-4 col-span-1">
            ...
            <p className='font-light text-sm mt-1'>{format(new Date(),"dd/MM/yyyy")}</p>
            </div>
            <p className='font-light text-sm'>Box Cricket</p>
          </Card>
        </div>
        <div className='grid grid-cols-1 gap-5 md:grid-cols-2'>
        <BarChartComponent className='' data={chartData} bookings={100} percentageIncrease={1.5}/>
        <RecentBookings/>
        </div>
